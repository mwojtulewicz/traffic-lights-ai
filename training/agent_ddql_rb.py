...
from traffic_envs import Environment_Trafic_Lights
...
env = Environment_Trafic_Lights(state_class=STATE_CLASS, reward_class=REWARD_CLASS, max_steps=MAX_STEPS,
...
writer = SummaryWriter(log_dir=f'runs/{env.__class__.__name__}/run{START_TIME}', comment=f'{START_TIME}')
...
for episode in range(0,NUM_EPISODES):
    ...
    for t in tqdm.trange(MAX_TIMESTEPS, position=0, leave=True):
        ...
        if len(replay_buffer)>MIN_BUFFER_LENGTH and t%TRAIN_FREQ==0:
            ...
            with torch.no_grad():
                _qvalues_next_all = Qtarget(_next_states)
                ...
                # _qvalues_expected = _qvalues_expected.detach()
